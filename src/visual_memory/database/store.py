...
class DatabaseStore:
    ...
    def _create_tables(self):
        ...
        self._apply_optional_migration("ALTER TABLE items ADD COLUMN vlm_description TEXT")
    ...
    def add_item(
        ...
        cur = self._conn.execute(
            "INSERT INTO items "
            "(label, combined_embedding, ocr_text, image_path, confidence, timestamp, label_embedding, ocr_embedding, visual_attributes, vlm_description) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (
                label,
                blob,
    ...
    def get_items_metadata(self, label: Optional[str] = None) -> list[dict]:
        ...
        if label is not None:
            rows = self._conn.execute(
                "SELECT id, label, confidence, ocr_text, image_path, visual_attributes, vlm_description, timestamp FROM items "
        ...
        result = []
        for row_id, lbl, confidence, ocr_text, image_path, visual_attributes, vlm_description, timestamp in rows:
            try:
    ...
    @_synchronized
    def update_item_image_path(self, label: str, image_path: str) -> None:
        """Legacy helper: update the newest item row for label after upload persist."""
        self._conn.execute(
            "UPDATE items SET image_path = ? WHERE id = "
            "(SELECT MAX(id) FROM items WHERE label = ?)",
            (image_path or "", label),
        )
        self._conn.commit()

    @_synchronized
    def update_item_vlm_description(self, label: str, description: str) -> None:
        """Legacy helper: store teach-time VLM description on newest row for label."""
        self._conn.execute(
            "UPDATE items SET vlm_description = ? WHERE id = "
            "(SELECT MAX(id) FROM items WHERE label = ?)",
    ...
    @_synchronized
    def update_item_image_path_by_id(self, item_id: int, image_path: str) -> None:
        """Update image_path for a specific item row id."""
        self._conn.execute(
            "UPDATE items SET image_path = ? WHERE id = ?",
            (image_path or "", int(item_id)),
        )
        self._conn.commit()

    @_synchronized
    def update_item_vlm_description_by_id(self, item_id: int, description: str) -> None:
        """Store teach-time VLM description for a specific item row id."""
        self._conn.execute(
            "UPDATE items SET vlm_description = ? WHERE id = ?",
            (description or "", int(item_id)),
        )
        self._conn.commit()

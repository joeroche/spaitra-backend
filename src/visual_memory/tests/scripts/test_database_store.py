...
def test_add_item_stores_vlm_description():
    db, _ = _make_db()
    db.add_item(
        label="wallet",
        combined_embedding=make_embedding(0),
        confidence=0.8,
        timestamp=time.time(),
        vlm_description="A black wallet on a table.",
    )
    items = db.get_items_metadata(label="wallet")
    assert items[0]["vlm_description"] == "A black wallet on a table."


def test_update_item_vlm_description():
    db, _ = _make_db()
    db.add_item(label="wallet", combined_embedding=make_embedding(0), confidence=0.8, timestamp=time.time())
    db.update_item_vlm_description("wallet", "A folded wallet.")
    items = db.get_items_metadata(label="wallet")
    assert items[0]["vlm_description"] == "A folded wallet."


def test_update_item_image_path():
    db, _ = _make_db()
    db.add_item(label="wallet", combined_embedding=make_embedding(0), confidence=0.8, timestamp=time.time())
    db.update_item_image_path("wallet", "/tmp/example.jpg")
    items = db.get_items_metadata(label="wallet")
    assert items[0]["image_path"] == "/tmp/example.jpg"


def test_update_item_vlm_description_by_id_targets_exact_row():
    db, _ = _make_db()
    id1 = db.add_item(label="wallet", combined_embedding=make_embedding(0), confidence=0.8, timestamp=1.0)
    id2 = db.add_item(label="wallet", combined_embedding=make_embedding(1), confidence=0.7, timestamp=2.0)
    db.update_item_vlm_description_by_id(id1, "older-row-description")
    items = db.get_items_metadata(label="wallet")
    by_id = {item["id"]: item for item in items}
    assert by_id[id1]["vlm_description"] == "older-row-description"
    assert by_id[id2]["vlm_description"] == ""


def test_update_item_image_path_by_id_targets_exact_row():
    db, _ = _make_db()
    id1 = db.add_item(label="wallet", combined_embedding=make_embedding(0), confidence=0.8, timestamp=1.0)
    id2 = db.add_item(label="wallet", combined_embedding=make_embedding(1), confidence=0.7, timestamp=2.0)
    db.update_item_image_path_by_id(id1, "/tmp/older-row.jpg")
    items = db.get_items_metadata(label="wallet")
    by_id = {item["id"]: item for item in items}
    assert by_id[id1]["image_path"] == "/tmp/older-row.jpg"
    assert by_id[id2]["image_path"] == ""
...
for name, fn in [
    ...
    ("db:add_item_vlm_description", test_add_item_stores_vlm_description),
    ("db:update_item_vlm_description", test_update_item_vlm_description),
    ("db:update_item_image_path", test_update_item_image_path),
    ("db:update_item_vlm_description_by_id", test_update_item_vlm_description_by_id_targets_exact_row),
    ("db:update_item_image_path_by_id", test_update_item_image_path_by_id_targets_exact_row),

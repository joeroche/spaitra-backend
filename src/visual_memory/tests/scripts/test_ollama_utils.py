...
def test_extract_item_intent_unclear_returns_none():
    with patch.object(_ou, "_chat_raw", return_value='{"intent": "unclear"}'):
        result = _ou.extract_item_intent("query")
    assert result is None
...
def test_answer_ocr_question_returns_targeted_answer():
    with patch.object(_ou, "_chat_raw", return_value="The doctor is Dr. Smith."):
        result = _ou.answer_ocr_question("Doctor: Dr. Smith", "who is the doctor?")
    assert result == "The doctor is Dr. Smith."


def test_answer_ocr_question_none_on_cb_open():
    with patch.object(_ou, "_chat_raw", return_value=None):
        result = _ou.answer_ocr_question("Doctor: Dr. Smith", "who is the doctor?")
    assert result is None


def test_answer_ocr_question_clips_long_text():
    captured = {}
    def _capture(prompt, max_retries=None, json_mode=False):
        captured["prompt"] = prompt
        return "It says hello."
    with patch.object(_ou, "_chat_raw", side_effect=_capture):
        _ou.answer_ocr_question("x" * 4000, "what does it say?")
...
for name, fn in [
    ...
    ("ollama:ocr_qa_answer", test_answer_ocr_question_returns_targeted_answer),
    ("ollama:ocr_qa_none", test_answer_ocr_question_none_on_cb_open),
    ("ollama:ocr_qa_clips", test_answer_ocr_question_clips_long_text),
]:
    _runner.run(name, fn)

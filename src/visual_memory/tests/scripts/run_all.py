...
Suites:
    ...
    e2e     - live endpoint checks (requires running service + env)
...
_UNIT_TESTS = [
    ("test_audio_utils",     "unit", ["audio", "transcribe"]),
    ("test_metrics_logger",  "unit", ["logging", "metrics"]),
...
_E2E_TESTS = [
    ("test_audio_formats_e2e", "e2e", ["e2e", "audio", "transcribe"]),
]

_STRESS_TESTS = [
    ("test_runpod_runtime", "stress", ["stress", "runtime", "runpod"]),
    ("test_live_stress_api", "stress", ["stress", "live", "api"]),
    ("test_live_stress_ocr", "stress", ["stress", "live", "ocr"]),
    ("test_live_stress_mixed", "stress", ["stress", "live", "ws", "mixed"]),
]
...
def main():
    ...
    parser.add_argument("--suite", choices=["unit", "api", "e2e", "system", "stress", "pentest", "all"], default="all")

...
def test_voice_invalid_format():
    ...
    from visual_memory.utils.audio_utils import AudioDecodeError

    old_loader = _tr.load_audio_bytes
    _tr.load_audio_bytes = lambda b, target_sr=16000: (_ for _ in ()).throw(AudioDecodeError("bad format"))
    try:
        resp = client.post("/voice", data=b"RIFF" + b"x" * 40, content_type="audio/wav")
    finally:
        _tr.load_audio_bytes = old_loader
    assert_status(resp, 400)
    data = resp.get_json()
    assert data.get("code") == "stt_decode_failed"

...
def test_audio_teach_without_label_prompts_clarification():
    client, sio, db, stub, cleanup = _make_ws_test_app(empty_db=True)
    try:
        client.connect()
        client.get_received()

        stub.set_result("teach")
        client.emit("audio", {"audio": _ogg_audio()})
        received = client.get_received()

        tts_events = [e for e in received if e["name"] == "tts"]
        assert tts_events, "missing teach clarification tts"
        assert "what should i call this item" in tts_events[-1]["args"][0].get("narration", "").lower()

        state_events = [e for e in received if e["name"] == "session_state"]
        assert state_events, "missing session state update"
        assert state_events[-1]["args"][0].get("context", {}).get("pending_voice_action") == "remember_label"
    finally:
        cleanup()


def test_audio_find_without_target_prompts_clarification():
    client, sio, db, stub, cleanup = _make_ws_test_app(seed_fn=_seed_items)
    try:
        client.connect()
        client.get_received()

        stub.set_result("find")
        client.emit("audio", {"audio": _ogg_audio()})
        received = client.get_received()

        tts_events = [e for e in received if e["name"] == "tts"]
        assert tts_events, "missing find clarification tts"
        assert "what would you like me to find" in tts_events[-1]["args"][0].get("narration", "").lower()

        state_events = [e for e in received if e["name"] == "session_state"]
        assert state_events, "missing session state update"
        assert state_events[-1]["args"][0].get("context", {}).get("pending_voice_action") == "find_target"
    finally:
        cleanup()
...
def test_awaiting_confirmation_ambiguous_reprompts():
    from visual_memory.api.voice_session import _sessions
    ...
    try:
        ...
        session.context["pending_confirmation"] = {"action": "delete_item", "label": "wallet"}

        stub.set_result("maybe")
        client.emit("audio", {"audio": _ogg_audio()})
        received = client.get_received()

        tts_events = [e for e in received if e["name"] == "tts"]
        assert tts_events, "missing confirmation clarification tts"
        assert "please say yes to confirm or no to cancel" in tts_events[-1]["args"][0].get("narration", "").lower()

        state_events = [e for e in received if e["name"] == "session_state"]
        assert state_events[-1]["args"][0].get("current_mode") == "awaiting_confirmation"
    finally:
        cleanup()
...
def test_shortcut_submit_emits_ack_and_runs_normal_scan_turn():
    """shortcut_submit emits transcription and still forces the normal scan turn."""
    import visual_memory.api.pipelines as _pm
    ...
    try:
        ...
        assert names.index("shortcut_ack") < names.index("transcription")
        assert names.index("transcription") < names.index("action_result")
        assert names.index("action_result") < names.index("tts")
        assert names.index("tts") < names.index("session_state")
    finally:
...
def test_audio_dispatch_exception_returns_error_event():
    """Unexpected audio-turn exceptions should surface as an explicit error event."""
    import visual_memory.api.routes.voice_ws as _vws
    ...
    try:
        client.connect()
        client.get_received()
        original_run_scan = _vws._run_scan
        _vws._run_scan = lambda image_bytes, focal_length: (_ for _ in ()).throw(RuntimeError("boom"))
        ...
        try:
            ...
            _vws._run_scan = original_run_scan

        names = _event_names(received)
        assert "transcription" in names, "transcription should still be emitted before dispatch failure"
        assert "error" in names, "missing explicit error event for dispatch failure"
        errors = [e for e in received if e["name"] == "error"]
        assert errors[-1]["args"][0].get("code") == "internal_error"
    finally:
        cleanup()
...
for name, fn in [
    ...
    ("ws:teach_label_clarify", test_audio_teach_without_label_prompts_clarification),
    ("ws:find_target_clarify", test_audio_find_without_target_prompts_clarification),
    ("ws:awaiting_location", test_awaiting_location_transitions_to_idle),
    ("ws:confirm_clarify", test_awaiting_confirmation_ambiguous_reprompts),
    ...
    ("ws:audio_dispatch_exception_error", test_audio_dispatch_exception_returns_error_event),

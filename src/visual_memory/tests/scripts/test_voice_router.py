...
def test_classify_item_intent():
    ...
    assert classify_item_intent("nonsense command") is None
...
def test_voice_teach_without_label_prompts_clarification():
    resp = client.post(
        "/voice",
        json={
            "text": "teach",
            "state": {"current_mode": "onboarding_teach", "context": {"onboarding_phase": "teach_1"}},
        },
    )
    assert_status(resp, 200)
    data = resp.get_json()
    assert data.get("request_type") == "remember"
    assert data.get("result", {}).get("action") == "clarify_label"
    assert data.get("next_state") == "onboarding_teach"
    assert data.get("state_contract", {}).get("context", {}).get("pending_voice_action") == "remember_label"


def test_voice_find_without_target_prompts_clarification():
    resp = client.post("/voice", json={"text": "find", "state": {"current_mode": "idle"}})
    assert_status(resp, 200)
    data = resp.get_json()
    assert data.get("request_type") == "find"
    assert data.get("result", {}).get("action") == "clarify_find"
    assert data.get("next_state") == "idle"
    assert data.get("state_contract", {}).get("context", {}).get("pending_voice_action") == "find_target"


def test_voice_awaiting_confirmation_ambiguous_reprompts():
    resp = client.post(
        "/voice",
        json={
            "text": "maybe",
            "state": {"current_mode": "awaiting_confirmation", "context": {"action": "delete_item", "target": "wallet"}},
        },
    )
    assert_status(resp, 200)
    data = resp.get_json()
    assert data.get("request_type") == "confirm_action"
    assert data.get("result", {}).get("action") == "clarify_confirmation"
    assert data.get("next_state") == "awaiting_confirmation"


def test_voice_focused_item_ambiguous_query_prompts_clarification():
    resp = client.post(
        "/voice",
        json={
            "text": "help",
    ...
    data = resp.get_json()
    assert data.get("request_type") == "item_ask"
    assert data.get("result", {}).get("action") == "clarify_intent"


def test_voice_policy_isolates_onboarding_teach_context():
    policy = resolve_voice_policy("onboarding_teach", {})
    assert policy.get("include_known_labels") is False
    assert policy.get("include_recent_rooms") is False
    assert policy.get("include_common_query_terms") is False


def test_voice_policy_keeps_find_labels_but_drops_rooms_during_find_clarification():
    policy = resolve_voice_policy("idle", {"pending_voice_action": "find_target"})
    assert policy.get("include_known_labels") is True
    assert policy.get("include_recent_rooms") is False
    assert policy.get("include_common_query_terms") is True
...
for name, fn in [
    ...
    ("voice_router:teach_label_clarify", test_voice_teach_without_label_prompts_clarification),
    ("voice_router:find_target_clarify", test_voice_find_without_target_prompts_clarification),
    ("voice_router:confirm_clarify", test_voice_awaiting_confirmation_ambiguous_reprompts),
    ("voice_router:item_intent_clarify", test_voice_focused_item_ambiguous_query_prompts_clarification),
    ("voice_router:onboarding_policy_isolation", test_voice_policy_isolates_onboarding_teach_context),
    ("voice_router:find_policy_isolation", test_voice_policy_keeps_find_labels_but_drops_rooms_during_find_clarification),

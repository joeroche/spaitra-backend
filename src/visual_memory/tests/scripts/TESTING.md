...
# Live endpoint checks (requires running backend + TEST_BASE_URL/API_KEY as needed)
python -m visual_memory.tests.scripts.run_all --suite e2e

# RunPod runtime + live stress suite (requires TEST_BASE_URL/API_KEY and usually
# the real server environment for runtime checks)
...
### Live stress scripts
```bash
# Runtime invariants on the real RunPod host
python -m visual_memory.tests.scripts.test_runpod_runtime

# Concurrent API mix against the live base URL
TEST_BASE_URL=$BASE API_KEY=$API_KEY \
  python -m visual_memory.tests.scripts.test_live_stress_api

# Direct OCR concurrency matrix against the local OCR service
OCR_BASE_URL=http://127.0.0.1:8001 OCR_API_KEY=$API_KEY \
  python -m visual_memory.tests.scripts.test_live_stress_ocr

# Mixed HTTP + websocket soak
TEST_BASE_URL=$BASE API_KEY=$API_KEY STRESS_MIXED_CLIENTS=3 \
  python -m visual_memory.tests.scripts.test_live_stress_mixed
```

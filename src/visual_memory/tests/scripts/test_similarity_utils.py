...
def test_find_match_dynamic_threshold_grouped_max_picks_best_exemplar():
    # Two exemplars for "wallet": one close, one far. One exemplar for "keys": medium.
    # max mode should pick wallet (best exemplar ~1.0 beats keys ~medium).
    query = _norm(0)
    db = [
        ("wallet", _norm(0)),   # identical to query -> sim ~1.0
        ("wallet", _norm(42)),  # different -> low sim
        ("keys",   _norm(1)),   # different label -> some sim
    ]
    label, sim, margin = find_match_dynamic_threshold(
        query, db, lambda _: 0.0, score_aggregation_mode="max"
    )
    assert label == "wallet"
    assert sim > 0.9


def test_find_match_dynamic_threshold_grouped_top_2_avg_falls_back_single_exemplar():
    # "wallet" has only 1 exemplar; top_2_avg must fall back to max (single sim).
    query = _norm(0)
    db = [
        ("wallet", _norm(0)),  # sim ~1.0
        ("keys",   _norm(1)),  # lower sim
    ]
    label, sim, margin = find_match_dynamic_threshold(
        query, db, lambda _: 0.0, score_aggregation_mode="top_2_avg"
    )
    assert label == "wallet"
    assert sim > 0.9


def test_find_match_dynamic_threshold_grouped_top_2_avg_lowers_score():
    # "wallet" has 2 exemplars: one identical (sim 1.0), one very different (sim ~0).
    # top_2_avg for wallet ~ (1.0 + ~0) / 2 ~ 0.5.
    # "keys" has 1 exemplar with sim set to 0.8 by construction.
    # top_2_avg should prefer keys (0.8) over wallet (~0.5) in this case.
    query = F.normalize(torch.tensor([[1.0, 0.0, 0.0]]), dim=1)
    wallet_close = F.normalize(torch.tensor([[1.0, 0.0, 0.0]]), dim=1)   # sim ~1.0
    wallet_far   = F.normalize(torch.tensor([[0.0, 1.0, 0.0]]), dim=1)   # sim ~0.0
    keys_medium  = F.normalize(torch.tensor([[0.6, 0.8, 0.0]]), dim=1)   # sim ~0.6
    db = [
        ("wallet", wallet_close),
        ("wallet", wallet_far),
        ("keys",   keys_medium),
    ]
    label_max, _, _ = find_match_dynamic_threshold(
        query, db, lambda _: 0.0, score_aggregation_mode="max"
    )
    label_avg, _, _ = find_match_dynamic_threshold(
        query, db, lambda _: 0.0, score_aggregation_mode="top_2_avg"
    )
    # max: wallet wins (best exemplar 1.0 > keys 0.6)
    assert label_max == "wallet"
    # top_2_avg: wallet avg ~0.5, keys ~0.6 -> keys wins
    assert label_avg == "keys"
...
for name, fn in [
    ...
    ("sim:grouped_max_picks_best_exemplar", test_find_match_dynamic_threshold_grouped_max_picks_best_exemplar),
    ("sim:grouped_top_2_avg_fallback_single", test_find_match_dynamic_threshold_grouped_top_2_avg_falls_back_single_exemplar),
    ("sim:grouped_top_2_avg_lowers_outlier_score", test_find_match_dynamic_threshold_grouped_top_2_avg_lowers_score),

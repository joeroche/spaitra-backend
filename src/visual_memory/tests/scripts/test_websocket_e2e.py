...
Usage:
  TEST_BASE_URL=https://your-test-host.example \
  API_KEY=... \
...
class WsRecorder:
    ...
    def emit_shortcut_submit(self, shortcut: str, audio_bytes: bytes, image_bytes: bytes | None = None) -> None:
        payload: dict[str, Any] = {
            "shortcut": shortcut,
            "audio": base64.b64encode(audio_bytes).decode("ascii"),
        }
        if image_bytes is not None:
            payload["image"] = base64.b64encode(image_bytes).decode("ascii")
        self.client.emit("shortcut_submit", payload)
...
def _test_onboarding_flow_skeleton(cfg: dict, assets: dict) -> None:
    ...
    try:
        ...
        marker = ws.mark()
        ws.emit_shortcut_submit("scan", assets["audio_scan"], image_bytes=assets["scan_image"])
...
def _test_focused_mode_voice_feedback(cfg: dict, assets: dict) -> None:
    ...
    try:
        ...
        marker = ws.mark()
        ws.emit_audio(assets["audio_scan"], image_bytes=assets["scan_image"])
        step_timeout = max(cfg["event_timeout"], 90.0)
        assert ws.wait_event("transcription", marker, step_timeout) is not None, "scan missing transcription"
        scan_result = ws.wait_event(
            "action_result",
            marker,
            step_timeout,
            predicate=lambda d: isinstance(d, dict) and d.get("type") == "scan",
        )

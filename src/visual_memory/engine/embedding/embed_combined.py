    ...
    *,
    ...
    if text_emb is None:
        ...
        if text_confidence is not None:
            # Scale text contribution by OCR confidence so that blurry or low-quality
            # OCR text does not override visual signal.
            # conf=0.0 -> text_scale=0 (image only); conf=1.0 -> text_scale=weight+boost.
            conf = min(max(float(text_confidence), 0.0), 1.0)
            text_scale = conf * max(0.0, float(text_weight))
            text_scale += max(0.0, float(text_high_confidence_boost)) * conf
        else:
            # No confidence supplied: use base weight unchanged (backward compat).

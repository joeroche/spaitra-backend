...
_ALIAS_THRESHOLD = 0.92


def _check_alias_warning(
    combined_emb: torch.Tensor,
    new_label: str,
    existing_items: list,
) -> list:
    """Return list of {label, similarity} for existing labels whose embeddings
    are suspiciously close to combined_emb (above _ALIAS_THRESHOLD).

    Detects duplicate-label situations (same item taught under two names) and
    near-identical items that will confuse matching. Returns one entry per
    label, using the maximum pairwise similarity across all exemplars.
    """
    a = combined_emb if combined_emb.dim() == 2 else combined_emb.unsqueeze(0)
    by_label: dict[str, float] = {}
    for item in existing_items:
        label = item.get("label", "")
        if not label or label == new_label:
            continue
        db_emb = item.get("combined_embedding")
        if db_emb is None:
            continue
        b = db_emb if db_emb.dim() == 2 else db_emb.unsqueeze(0)
        try:
            sim = float(cosine_similarity(a.to(b.device), b))
        except Exception:
            continue
        if sim > by_label.get(label, -1.0):
            by_label[label] = sim
    return [
        {"label": lbl, "similarity": round(sim, 4)}
        for lbl, sim in sorted(by_label.items(), key=lambda x: x[1], reverse=True)
        if sim >= _ALIAS_THRESHOLD
    ]
...
class RememberPipeline:
    ...
    def detect_score_batch(
        ...
        blurs = [blur_score(img) for img in images]
    ...
    def run(self, image_path: Union[str, Path], prompt: str):
        ...
        # Check for near-duplicate labels before writing.
        # Warns when the new embedding is too close to an existing different label
        # (duplicate teach, same item stored under two names, or genuinely similar objects).
        existing_items = self.db.get_all_items()
        alias_warnings = _check_alias_warning(combined, label, existing_items)
        if alias_warnings:
            _log.warning({
                "event": "remember_alias_warning",
                "label": label,
                "similar_labels": alias_warnings,
            })
        ...
        result = {
            ...
            "ocr_text": ocr_text,
            "ocr_confidence": round(ocr_confidence, 4),
            "alias_warning": alias_warnings if alias_warnings else None,
        }

...
from visual_memory.pipelines.match_candidate import MatchCandidate
...
class ScanPipeline:
    ...
    def _retrieve(
        ...
        """RETRIEVE stage: rank all DB entries, apply threshold+margin, return candidate.

        Returns a MatchCandidate with label=None when nothing passes the gate.
        top_k always contains the k highest-similarity entries regardless of the gate.
        """
        settings = _runtime_settings()
        learning_active = bool(self._enable_learning and self._head_trained)
        top_k_list = find_top_k(projected_query, projected_db, k=k)
        match_label, similarity, margin = find_match_dynamic_threshold(
            projected_query,
            projected_db,
            lambda label: _match_threshold_for_label(label, learning_active),
            lambda label: _match_margin_for_label(label),
    ...
    def _verify(self, candidate: MatchCandidate) -> MatchCandidate:
        """VERIFY stage: no-op placeholder for Branch B verifier integration.

        Branch B will replace this with LightGlue or DINO patch verification.
        Returns the candidate unchanged.
        """
        return candidate

    def _decide(self, candidate: MatchCandidate) -> Optional[str]:
        """DECIDE stage: return the matched label or None.

        Currently a direct pass-through of the RETRIEVE decision. Branch A will
        add evidence-based accept/uncertain/reject logic here (Step A.4).
        """
        return candidate.label if candidate.accepted() else None
    ...
    def run(self, query_image, scan_id: Optional[str] = None, focal_length_px: Optional[float] = None):
        ...
        for i, (box, score) in enumerate(zip(boxes, scores)):
            ...
            is_document = is_document_like_label(ocr_text[:40] if ocr_text else "")
            match_t0 = time.monotonic()
            candidate = self._retrieve(projected_query, projected_db, is_document=is_document)
            candidate = self._verify(candidate)
            matched_label = self._decide(candidate)
            similarity = candidate.similarity
            similarity_margin = candidate.margin
            match_path = candidate.db_path

...
class Settings:
    ...
    # Top1-top2 similarity margin gate. Set to 0.0 to disable (recommended).
    # A non-zero margin rejects matches where top1 and top2 are close, which
    # permanently silences similar items in the DB (e.g. two wallets) because
    # the near-tie means no match is ever returned, so no feedback triplets
    # accumulate and the projection head can never learn to separate them.
    # Use the margin value as a signal in _decide() instead of a hard gate.
    scan_similarity_margin: float = 0.0
    scan_similarity_margin_document: Optional[float] = 0.0
    ...
    # Grounded OCR question answering in /item/ask. Falls back to stored text.
    llm_ocr_qa_enabled: bool = field(default_factory=lambda: os.environ.get("LLM_OCR_QA_ENABLED", "1") != "0")
    # Global /ask multi-step agent. Disabled by default until server validation.
    llm_agent_enabled: bool = field(default_factory=lambda: os.environ.get("LLM_AGENT_ENABLED", "0") != "0")
    llm_agent_max_steps: int = field(default_factory=lambda: int(os.environ.get("LLM_AGENT_MAX_STEPS", "3")))
    llm_agent_timeout_seconds: float = field(default_factory=lambda: float(os.environ.get("LLM_AGENT_TIMEOUT_SECONDS", "3.0")))
    ...
    ollama_model_accurate: str = field(default_factory=lambda: os.environ.get("OLLAMA_MODEL_ACCURATE", "llama3.1:8b"))
    ollama_agent_model: str = field(default_factory=lambda: os.environ.get("OLLAMA_AGENT_MODEL", ""))
    ...
    def get_ollama_agent_model(self) -> str | None:
        if self.ollama_agent_model:
            return self.ollama_agent_model
        return self.get_ollama_model("accurate")
    ...
    # Multi-prototype retrieval scoring.
    # "max": highest cosine similarity across all stored exemplars for a label.
    #        This is the default and matches implicit prior behavior (rank-1 in find_top_k).
    # "top_2_avg": mean of top-2 exemplar similarities per label. More robust when
    #              one DB exemplar is an outlier (bad teach crop); requires >= 2 exemplars
    #              or falls back to max automatically.
    # Do NOT average the embeddings themselves ("centroid"). Centroid of differently-
    # oriented or flipped exemplars yields a ghost vector that is farther from both
    # test images than either individual exemplar, destroying the diversity captured
    # by multiple teaches. See ACCURACY_HARDENING_PLAN.log Step 1.3 for rationale.
    score_aggregation_mode: str = "max"
    # Centroid DB mode: build DB as one averaged embedding per label instead of N
    # separate exemplars. Purely experimental -- expected to underperform "max" on
    # asymmetric objects (keys, glasses, wallets, receipts at varying angles).
    # Requires a DB rebuild; cannot be toggled at query time.
    centroid_db_mode: bool = False

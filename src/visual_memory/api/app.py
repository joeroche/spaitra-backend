...
def create_app():
    ...
    def _check_memory_pressure():
        if request.endpoint and request.endpoint.startswith("health."):
            return
        state = _memory_monitor.check_memory()
        if _memory_monitor.is_oom_risk(threshold=0.90, state=state):
            _memory_monitor.log_memory_state(level="critical", state=state)
            return jsonify({
                "error": "server_overloaded",
                "message": "Memory exhausted, try again later",
            }), 503
        if _memory_monitor.suggest_throttle(state=state):
            _memory_monitor.log_memory_state(level="warning", state=state)
    ...
    def _check_api_key():
        ...
        if request.endpoint is None:
            return
        if request.endpoint.startswith("health."):
            return

...
def process_item_ask_request(
    ...
    if resolved_intent is None:
        ...
        if resolved_intent is None:
            return {
                ...
                "label": label,
                "query": query,
                "narration": "Do you want me to read it, describe it, rename it, or tell you where it is?",
                "options": ["read_ocr", "describe", "rename", "find"],
            }, 200
    ...
    if resolved_intent == "ocr_question":
        ...
        if get_settings().llm_ocr_qa_enabled:
            answer = answer_ocr_question(normalized, query, state_context=state_context)
            if answer:
                _log.info({
                    "event": "ocr_qa_hit",
                    "label": label,
    ...
    if resolved_intent == "describe":
        ...
        stored_desc = (item.get("vlm_description") or "").strip()
        if stored_desc:
            return {
                "action": "describe",
                "label": label,
                "narration": stored_desc,
                "method": "vlm_cached",
                "latency_ms": round((time.monotonic() - t0) * 1000),
            }, 200

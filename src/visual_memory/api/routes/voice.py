...
def classify_item_intent(query: str) -> str | None:
...
def _process_confirm_action_request(command_text: str, context: dict) -> tuple[dict, int]:
    confirmation = resolve_confirmation(command_text)
    if confirmation is None:
        return {
            ...
            "narration": "Please say yes to confirm or no to cancel.",
        }, 200
    return {
        ...
        "target_action": context.get("action"),
        "target": context.get("target"),
        "narration": "Confirmed." if confirmation else "Canceled.",
    }, 200
...
def voice():
    ...
    if forced_type:
        ...
        classification = resolve_pending_voice_action(command_text, state_context) or classify_command(
            command_text,
            {"current_mode": mode},
        )
    ...
    if classification.get("error"):
        ...
        state_context.pop("pending_voice_action", None)
        narration_map = {
        ...
        prompt = normalize_teach_reply(prompt)
        if needs_label_clarification(prompt):
            state_context["pending_voice_action"] = "remember_label"
            result, status = {
                "action": "clarify_label",
                "label": "",
                "narration": "What should I call this item?",
            }, 200
            next_state = mode
        else:
            state_context.pop("pending_voice_action", None)
        ...
        if status == 200:
            if isinstance(result, dict) and result.get("action") == "clarify_find":
                state_context["pending_voice_action"] = "find_target"
                next_state = mode
            else:
                state_context.pop("pending_voice_action", None)
                next_state = "idle"
        ...
        if status == 200:
            next_state = mode if isinstance(result, dict) and result.get("action") == "clarify_confirmation" else "idle"

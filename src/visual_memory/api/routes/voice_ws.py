...
from visual_memory.utils.voice_command_utils import (
    extract_teach_label,
    needs_find_target_clarification,
    needs_label_clarification,
    normalize_teach_reply,
    resolve_confirmation,
    resolve_pending_voice_action,
)
...
def _emit_logged(name: str, payload, *, sid: str | None = None, level: str = "info") -> None:
    emit(name, payload)
    ...
    if isinstance(payload, dict):
        ...
        if name == "tts":
            record["next_state"] = payload.get("next_state")
            record["narration_len"] = len(str(payload.get("narration") or ""))
        elif name == "session_state":
            record["current_mode"] = payload.get("current_mode")
        elif name == "action_result":
            record["action_type"] = payload.get("type")
        elif name == "control":
            ...
            record["text_length"] = len(str(payload.get("text") or ""))
            record["context_policy"] = payload.get("context_policy")
        elif name == "shortcut_ack":
            record["shortcut"] = payload.get("shortcut")
            record["phase"] = payload.get("phase")
        elif name == "shortcut_error":
            record["shortcut"] = payload.get("shortcut")
            record["error_code"] = payload.get("code")
        elif name in {"listening", "listening_stopped", "shortcut_listening"}:
            record["state"] = payload.get("state")

    getattr(_log, level)(record)


def _emit_session_state(session: VoiceSession) -> None:
    _emit_logged("session_state", session_state_payload(session), sid=session.sid)
...
def _emit_error(code: str, message: str) -> None:
    ...
    _emit_logged("error", {"code": code, "message": message}, level="warning")


def _emit_shortcut_ack(shortcut: str, phase: str) -> None:
    if phase not in _SHORTCUT_ACK_PHASES:
        return
    _emit_logged("shortcut_ack", {"shortcut": shortcut, "phase": phase})


def _emit_shortcut_listening(*, shortcut: str, prompt: str, state: str) -> None:
    _emit_logged("shortcut_listening", {"shortcut": shortcut, "prompt": prompt, "state": state})
...
def _emit_shortcut_error(shortcut: str, code: str, message: str) -> None:
    ...
    _emit_logged(
        "shortcut_error",
        {"shortcut": shortcut, "code": normalized, "message": message},
        level="warning",
    )
...
def _emit_turn(
    ...
    *,
    ...
    if action_result is not None:
        _emit_logged("action_result", action_result, sid=session.sid)
    if control is not None:
        _emit_logged("control", control, sid=session.sid)
        _log.info({
    ...
    if tts_narration is not None:
        _emit_logged("tts", {"narration": tts_narration, "next_state": session.state}, sid=session.sid)
...
def _listening_prompt(session: VoiceSession) -> str:
    ...
    if state == "onboarding_teach":
        if session.context.get("pending_voice_action") == "remember_label":
            return "What should I call this item?"
    ...
    if session.context.get("pending_voice_action") == "find_target":
...
def _looks_like_supported_image(data: bytes) -> bool:
    ...
    if data.startswith(b"BM"):  # bmp
        return True
    if data.startswith((b"II*\x00", b"MM\x00*")):  # tiff
        return True
    if data[:4] == b"RIFF" and data[8:12] == b"WEBP":  # webp
        return True
    if data[4:8] == b"ftyp":  # heif/heic/avif family
        return True
    return False
...
def _parse_media_payload(data: dict) -> tuple[bytes, bytes | None, float | None, tuple[str, str] | None]:
    audio_bytes, audio_decode_error = _decode_audio(data.get("audio", b""))
    if audio_decode_error is not None:
        return b"", None, None, ("bad_payload", "Invalid audio payload.")

    raw_image = data.get("image")
    image_bytes, image_decode_error = _decode_image(raw_image)
    if image_decode_error is not None:
        return b"", None, None, ("bad_payload", "Invalid image payload.")

    if len(audio_bytes) > _MAX_WS_MEDIA_BYTES:
        return b"", None, None, ("bad_payload", "Audio payload exceeds 50MB limit.")
    if image_bytes is not None and len(image_bytes) > _MAX_WS_MEDIA_BYTES:
        return b"", None, None, ("bad_payload", "Image payload exceeds 50MB limit.")
    if image_bytes is not None and image_bytes and not _looks_like_supported_image(image_bytes):
        return b"", None, None, ("bad_payload", "Invalid image payload.")
    ...
    return audio_bytes, image_bytes, focal_length, None


def _emit_transcription_event(transcription: dict) -> str:
    command_text = (transcription.get("text") or "").strip()
    _emit_logged("transcription", {
        "text": command_text,
...
def _log_turn_latency(session: VoiceSession, transcription: dict) -> None:
...
def _handle_audio_turn(
    ...
    *,
    source: str,
    forced_command_text: str | None = None,
    shortcut: str | None = None,
) -> None:
    t0 = time.monotonic()

    audio_bytes, image_bytes, focal_length, payload_error = _parse_media_payload(data)
    if payload_error is not None:
        code, message = payload_error
        if shortcut is not None:
            _emit_shortcut_error(shortcut, code, message)
            return
    ...
    try:
        ...
        command_text = _emit_transcription_event(transcription)
        _log_turn_latency(session, transcription)
        _dispatch(session, forced_command_text or command_text, image_bytes, focal_length)
    except Exception as exc:
        _log.exception({
            "event": "ws_audio_turn_failed",
            "tag": LogTag.API,
            "sid": session.sid,
            "source": source,
            "state": session.state,
        ...
        _emit_error("internal_error", "Audio request failed.")
        return
    ...
    _log.info({
        ...
        "turn_total_latency_ms": duration_ms,
    })
...
def _extract_label(command_text: str, state_context: dict | None = None) -> str:
    """Extract the item label from a teach/remember command."""
    direct = extract_teach_label(command_text)
    if direct:
        return normalize_teach_reply(direct)
    return normalize_teach_reply(command_text)
...
def _run_find(command_text: str, state_context: dict | None = None) -> tuple[dict, int]:
    ...
    if needs_find_target_clarification(search_label):
        return {
            "action": "clarify_find",
            "label": "",
            "found": False,
            "narration": "What would you like me to find?",
            "sightings": [],
        }, 200
    db = get_database()
...
def _handle_confirmation(session: VoiceSession, command_text: str) -> None:
    decision = resolve_confirmation(command_text)
    if decision is None:
        _emit_turn(
            session,
            next_state="awaiting_confirmation",
            tts_narration="Please say yes to confirm or no to cancel.",
        )
        return
...
def _dispatch(
    ...
    pending_override = resolve_pending_voice_action(command_text, session.context)
    if pending_override is not None:
        command = pending_override.get("command")
        if command == "navigate_back":
            session.context.pop("pending_voice_action", None)
        ...
        if command == "open_settings":
            ...
            return
        if command == "clarify_label":
            _emit_turn(
                session,
                next_state=session.state,
                context_updates={"pending_voice_action": "remember_label"},
                tts_narration="What should I call this item?",
            )
            return
        if command == "clarify_find":
            _emit_turn(
                session,
                next_state=session.state,
                context_updates={"pending_voice_action": "find_target"},
                tts_narration="What would you like me to find?",
            )
            return
        if command == "remember":
            session.context.pop("pending_voice_action", None)
            intent = "remember"
            command_text = str(pending_override.get("label") or command_text)
        elif command == "find":
            session.context.pop("pending_voice_action", None)
            intent = "find"
            command_text = str(pending_override.get("query") or command_text)
        else:
    ...
    if intent == "scan":
        ...
        if image_bytes is None:
            _emit_turn(
                session,
                next_state="awaiting_image",
                context_updates={"pending_action": "remember", "label": label, "pending_voice_action": None},
                control={"action": "request_image"},
        ...
        narration = result.get("narration", "") if isinstance(result, dict) else ""
        if isinstance(result, dict) and result.get("action") == "clarify_find":
            _emit_turn(
                session,
                next_state=session.state,
                context_updates={"pending_voice_action": "find_target"},
                action_result={"type": "find", "data": result},
                tts_narration=narration or "What would you like me to find?",
            )
            return
        ...
        if session.context.get("onboarding_phase") == "ask_prompted":
            ...
            _emit_turn(
                session,
                next_state="idle",
                context_updates={"onboarding_phase": "", "pending_voice_action": None},
                action_result={"type": "find", "data": result},
            ...
            _emit_turn(
                session,
                next_state="idle",
                context_updates={"pending_voice_action": None},
                action_result={"type": "find", "data": result},
...
def register_events(sio: SocketIO) -> None:
    ...
    def on_audio(data):
        ...
        session = get_session(request.sid)
        _handle_audio_turn(session, data, source="audio")
    ...
    def on_chat_stop(data):
        ...
        session = get_session(request.sid)
        _emit_logged("listening_stopped", {"state": session.state}, sid=session.sid)
    ...
    def on_shortcut_submit(data):
        ...
        _handle_audio_turn(
            session,
            data,
            source="shortcut_submit",
            forced_command_text=_SHORTCUT_SCAN,
            shortcut=shortcut,
        )

...
def standardize_for_embedding(
    image: Image.Image,
    processor,
    *,
    return_tensor: bool = False,
):
    """Apply the same resize and pad that an AutoImageProcessor applies before embedding.

    Ensures benchmark and production use identical preprocessing so test-time
    crops are processed the same way as train-time teach crops.

    Args:
        image: Input PIL Image (RGB).
        processor: A HuggingFace AutoImageProcessor instance (e.g., from ImageEmbedder).
        return_tensor: If True, return the preprocessed tensor dict instead of PIL Image.
            Use True for direct model input; False (default) for auditing the crop.

    Returns:
        If return_tensor is False: PIL Image after resize/pad as the processor would apply.
        If return_tensor is True: the dict returned by processor(images=image).
    """
    ...
    if return_tensor:
        return processed
    # Convert pixel_values back to a PIL Image for visual inspection.
    pixel_values = processed["pixel_values"][0]  # (C, H, W) in [0, 1]
    # Undo normalization using the processor's image_mean and image_std if present.
    mean = getattr(processor, "image_mean", None)
    std = getattr(processor, "image_std", None)
    if mean is not None and std is not None:
        mean_t = _torch.tensor(mean, dtype=pixel_values.dtype).view(3, 1, 1)
        std_t = _torch.tensor(std, dtype=pixel_values.dtype).view(3, 1, 1)
        pixel_values = pixel_values * std_t + mean_t
    pixel_values = pixel_values.clamp(0.0, 1.0)
    np_img = (pixel_values.permute(1, 2, 0).numpy() * 255).astype("uint8")

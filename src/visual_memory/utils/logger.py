...
def _open_group_readable(path: str, flags: int) -> int:
...
def _ensure_group_readable(path: Path) -> None:
    try:
        path.chmod(0o660)
    except OSError:
        pass
...
def _open_crash_log(path: Path):
    fd = _open_group_readable(str(path), os.O_WRONLY | os.O_CREAT | os.O_APPEND)
    return os.fdopen(fd, "a", encoding="utf-8")


def _configure_sinks() -> None:
    _LOG_DIR.mkdir(parents=True, exist_ok=True)
    _ensure_group_readable(_LOG_DIR / "app.log")
    _ensure_group_readable(_LOG_DIR / "important.log")
...
def setup_crash_handler() -> None:
    ...
    _LOG_DIR.mkdir(parents=True, exist_ok=True)
    _ensure_group_readable(crash_path)
    # faulthandler handles SIGSEGV, SIGBUS, stack overflow, etc.
    faulthandler.enable(file=_open_crash_log(crash_path), all_threads=True)

    _orig_hook = sys.excepthook

    def _excepthook(exc_type, exc_val, exc_tb):
        import traceback
        with _open_crash_log(crash_path) as f:

...
class MemoryMonitor:
    def __init__(self) -> None:
        ...
        self._cache_ttl_s = max(float(os.environ.get("MEMORY_MONITOR_CACHE_TTL_SECONDS", "0.5")), 0.0)
        self._cache_lock = Lock()
        self._last_state: Optional[dict] = None
        self._last_state_t: float = 0.0

    def check_memory(self, *, force_refresh: bool = False) -> dict:
        now = time.monotonic()
        with self._cache_lock:
            if (
                ...
                and (now - self._last_state_t) <= self._cache_ttl_s
            ):
    ...
    def is_oom_risk(self, threshold: float = 0.85, *, state: Optional[dict] = None) -> bool:
        state = state or self.check_memory()
        return (state["combined_pct"] / 100.0) >= threshold

    def suggest_throttle(self, *, state: Optional[dict] = None) -> bool:
        state = state or self.check_memory()
    ...
    def log_memory_state(self, level: str = "warning", *, state: Optional[dict] = None) -> None:
        state = state or self.check_memory()
        payload = {
            "event": "memory_state",
            "tag": LogTag.VRAM,

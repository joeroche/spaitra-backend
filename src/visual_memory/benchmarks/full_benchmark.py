...
from visual_memory.utils.quality_utils import mean_luminance, estimate_text_likelihood, should_run_ocr
from visual_memory.utils.similarity_utils import cosine_similarity, detect_alias_candidates, find_match_dynamic_threshold, find_top_k
...
_DEFAULT_BENCHMARK_EMBED_BATCH_SIZE = 16
_YOLOE_DETECT_BATCH_SIZE = 32
_GDINO_DETECT_BATCH_SIZE = 16


def _benchmark_embed_batch_size() -> int:
    raw = os.environ.get("BENCHMARK_EMBED_BATCH_SIZE", "").strip()
    if raw:
        try:
            value = int(raw)
        except ValueError:
            value = _DEFAULT_BENCHMARK_EMBED_BATCH_SIZE
        return max(1, value)
    return _DEFAULT_BENCHMARK_EMBED_BATCH_SIZE
...
def _component_similarities(
    combined_a: torch.Tensor,
    combined_b: torch.Tensor,
    img_dim: int = 1024,
    text_dim: int = 512,
) -> tuple[float, float]:
    """Split 1536-dim embeddings and return (img_cosine_sim, text_cosine_sim).

    img_dim: first img_dim dimensions are the visual (DINOv3) component.
    text_dim: next text_dim dimensions are the text (CLIP) component.
    Returns 0.0 for degenerate (zero-norm) slices.
    """
    a_img = combined_a[:img_dim]
    b_img = combined_b[:img_dim]
    a_txt = combined_a[img_dim:img_dim + text_dim]
    b_txt = combined_b[img_dim:img_dim + text_dim]

    if float(a_img.norm()) < 1e-9 or float(b_img.norm()) < 1e-9:
        img_sim = 0.0
    else:
        img_sim = float(cosine_similarity(a_img, b_img).item())

    if float(a_txt.norm()) < 1e-9 or float(b_txt.norm()) < 1e-9:
        txt_sim = 0.0
    else:
        txt_sim = float(cosine_similarity(a_txt, b_txt).item())
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--no-detection",
        action="store_true",
        help="Skip YOLOE and GroundingDINO detector evaluation",
    )
    p.add_argument(
        "--depth-checkpoint",
        type=Path,
    ...
    p.add_argument(
        ...
        type=float,
        default=None,
        help="Override settings.ocr_text_likelihood_threshold for this run.",
    )
    return p.parse_args()
...
def _embed_rows(
    ...
    n = len(rows)
    batch_size = _benchmark_embed_batch_size()
    ...
    for batch_start in range(0, n, batch_size):
        ...
        for offset, row in enumerate(batch_rows):
            ...
            should_ocr = should_run_ocr(
                text_likelihood,
                lower_threshold=settings.ocr_text_likelihood_threshold,
                upper_threshold=settings.ocr_text_likelihood_upper_threshold,
                luminance=lum,
                blur_score=blur,
                rescue_threshold=settings.ocr_text_likelihood_rescue_threshold,
                rescue_min_luminance=settings.ocr_text_likelihood_rescue_min_luminance,
                rescue_min_blur_score=settings.ocr_text_likelihood_rescue_min_blur_score,
            )
            batch_items.append({
                "row": row,
                "fname": fname,
        ...
        img_batch = registry.img_embedder.batch_embed(batch_images)
        batch_lat_img = time.perf_counter() - t0
        per_item_lat_img = batch_lat_img / max(1, len(batch_items))

        ocr_results_by_index: Dict[int, dict] = {}
        lat_ocr_by_index: Dict[int, float] = {}
        if ocr_client is not None:
            ocr_indices = [idx for idx, item in enumerate(batch_items) if item["should_ocr"]]
            if ocr_indices:
                def _ocr_one(idx: int):
                    t0 = time.perf_counter()
                    result = ocr_client.recognize(batch_items[idx]["image"])
                    return idx, result, time.perf_counter() - t0
                with concurrent.futures.ThreadPoolExecutor(max_workers=4) as pool:
                    for idx, result, lat in pool.map(_ocr_one, ocr_indices):
                        ocr_results_by_index[idx] = result
                        lat_ocr_by_index[idx] = lat

        text_emb_by_index: Dict[int, torch.Tensor] = {}
        lat_txt_by_index: Dict[int, float] = {}
        text_inputs: List[tuple[int, str]] = []
        for idx, ocr in ocr_results_by_index.items():
            text = str((ocr or {}).get("text", "") or "")
            if text.strip():
                text_inputs.append((idx, text))

        if text_inputs:
            t0 = time.perf_counter()
            text_batch = registry.text_embedder.batch_embed_text([text for _, text in text_inputs])
            batch_lat_txt = time.perf_counter() - t0
            per_item_lat_txt = batch_lat_txt / max(1, len(text_inputs))
            for row_idx, (idx, _) in enumerate(text_inputs):
                text_emb_by_index[idx] = text_batch[row_idx:row_idx + 1]
                lat_txt_by_index[idx] = per_item_lat_txt

        for idx, item in enumerate(batch_items):
            row = item["row"]
            text_emb = text_emb_by_index.get(idx)
            emb = make_combined_embedding(img_batch[idx:idx + 1], text_emb)
            embedded[item["fname"]] = {
                "label": row["label"],
                ...
                "lat_embed_img": per_item_lat_img,
                "lat_ocr": lat_ocr_by_index.get(idx, 0.0),
                "lat_embed_txt": lat_txt_by_index.get(idx, 0.0),
                "darkness_level": round(item["lum"], 2),
                "is_dark": item["lum"] < settings.darkness_threshold,
                "blur_score": round(item["blur"], 2),
                "is_blurry": item["blur"] < BLUR_THRESHOLD,
                "text_likelihood": round(item["text_likelihood"], 3),
                "should_skip_ocr": not item["should_ocr"],
            }

        if monitor is not None and len(embedded) % 20 == 0:
...
def _load_split_manifest(path: Path, embedded: Dict[str, dict]) -> Tuple[List[str], List[str], List[str], dict]:
    ...
    if not train_set and not test_set and isinstance(payload.get("splits"), dict):
        train_set = []
        test_set = []
        for label, split in sorted(payload["splits"].items()):
            if not isinstance(split, dict):
                raise ValueError(f"invalid split entry for label {label!r}: {path}")
            train_set.extend(str(x) for x in split.get("train", []))
            test_set.extend(str(x) for x in split.get("test", []))
        db_set = []
        for label in sorted(payload["splits"]):
            label_train = sorted(
                f for f in train_set
                if embedded.get(f, {}).get("label") == label
            )
            preferred = [f for f in label_train if "_1ft_bright_clean." in f]
            if preferred:
                db_set.append(preferred[0])
            elif label_train:
                db_set.append(label_train[0])
...
def _eval_retrieval(
    ...
    for i, fname in enumerate(test_set):
        ...
        proj_emb = head.project(test_emb)
        t0 = time.perf_counter()
        ...
        top_k_pe = find_top_k(proj_emb, projected_db, k=10)
        top_k_labels = [lbl for lbl, _ in top_k_pe]
        true_rank = next(
            (i + 1 for i, lbl in enumerate(top_k_labels) if lbl == true_label), 0
        )
        top_3_hit = int(true_label in top_k_labels[:3])
        top_5_hit = int(true_label in top_k_labels[:5])
        top_10_hit = int(true_label in top_k_labels[:10])
        personalized_matched = int(pe_label is not None)
        personalized_match_label = pe_label or ""

        true_embs_pe = [emb for lbl, emb in projected_db if lbl == true_label]
        if true_embs_pe:
            best_true_emb = max(
                true_embs_pe, key=lambda e: float(cosine_similarity(proj_emb, e))
            )
            img_sim_pe, txt_sim_pe = _component_similarities(proj_emb, best_true_emb)
        else:
            img_sim_pe, txt_sim_pe = 0.0, 0.0
        ...
        results.append({
            ...
            "top_k_rank": true_rank,
            "top_3_hit": top_3_hit,
            "top_5_hit": top_5_hit,
            "top_10_hit": top_10_hit,
            "personalized_matched": personalized_matched,
            "personalized_match_label": personalized_match_label,
            "img_sim_pe": round(img_sim_pe, 6),
            "txt_sim_pe": round(txt_sim_pe, 6),
        })
...
def _compute_hardening_metrics(retrieval: List[dict]) -> dict:
    """Aggregate Phase 0.2 hardening metrics from per-row retrieval results."""
    n = max(len(retrieval), 1)
    top3 = sum(r.get("top_3_hit", 0) for r in retrieval)
    top5 = sum(r.get("top_5_hit", 0) for r in retrieval)
    top10 = sum(r.get("top_10_hit", 0) for r in retrieval)
    matched = sum(r.get("personalized_matched", 0) for r in retrieval)
    correct_among_matched = sum(
        r["personalized_correct"] for r in retrieval if r.get("personalized_matched", 0)
    )
    safe_count = sum(
        1 for r in retrieval
        if _classify_failure(r) in {"correct_match", "correct_abstention"}
    )
    lats = sorted(r["lat_retrieve_pe"] for r in retrieval)
    p50_idx = int(0.50 * len(lats))
    p95_idx = min(int(0.95 * len(lats)), len(lats) - 1)
    lat_p50 = lats[p50_idx] if lats else 0.0
    lat_p95 = lats[p95_idx] if lats else 0.0

    # Per-label accuracy
    label_buckets: Dict[str, List[int]] = {}
    for r in retrieval:
        lbl = str(r.get("label", "unknown"))
        label_buckets.setdefault(lbl, []).append(int(r.get("personalized_correct", 0)))
    per_label = {
        lbl: round(sum(hits) / max(len(hits), 1), 4)
        for lbl, hits in sorted(label_buckets.items())
    }

    # Per-distance accuracy
    dist_buckets: Dict[str, List[int]] = {}
    for r in retrieval:
        db = str(r.get("distance_bucket", "unknown"))
        dist_buckets.setdefault(db, []).append(int(r.get("personalized_correct", 0)))
    per_distance = {
        db: round(sum(hits) / max(len(hits), 1), 4)
        for db, hits in sorted(dist_buckets.items())
    }

    # Per-lighting accuracy
    light_buckets: Dict[str, List[int]] = {}
    for r in retrieval:
        lb = str(r.get("lighting_bucket", "unknown"))
        light_buckets.setdefault(lb, []).append(int(r.get("personalized_correct", 0)))
    per_lighting = {
        lb: round(sum(hits) / max(len(hits), 1), 4)
        for lb, hits in sorted(light_buckets.items())
    }

    return {
        "top_3_recall": round(top3 / n, 4),
        "top_5_recall": round(top5 / n, 4),
        "top_10_recall": round(top10 / n, 4),
        "accepted_match_rate": round(matched / n, 4),
        "accepted_precision": round(correct_among_matched / max(matched, 1), 4),
        "correct_accept_rate": round(
            sum(r["personalized_correct"] for r in retrieval) / max(matched, 1), 4
        ),
        "abstention_rate": round(1.0 - matched / n, 4),
        "safety_score": round(safe_count / n, 4),
        "latency_p50_retrieve_pe": round(lat_p50, 6),
        "latency_p95_retrieve_pe": round(lat_p95, 6),
        "per_label_accuracy": per_label,
        "per_distance_accuracy": per_distance,
        "per_lighting_accuracy": per_lighting,
    }


def _classify_failure(row: dict) -> str:
    """Assign failure taxonomy category to a retrieval result row.

    correct_match       - match returned and correct
    wrong_label_match   - match returned but wrong label (FP)
    threshold_miss      - true label rank-1 but below threshold/margin gate
    near_miss           - true label in top-3 but not rank-1
    correct_abstention  - no match, true label rank 4-10 (ambiguous)
    retrieval_miss      - true label not in top-10
    """
    matched = bool(row.get("personalized_matched", 0))
    correct = bool(row.get("personalized_correct", 0))
    rank = int(row.get("top_k_rank", 0))
    if matched and correct:
        return "correct_match"
    if matched and not correct:
        return "wrong_label_match"
...
def _eval_detection(
    ...
    items = list(test_set)
    images_by_fname = {fname: load_image(embedded[fname]["image_path"]) for fname in items}

    batch_size = _GDINO_DETECT_BATCH_SIZE
    for batch_start in range(0, len(items), batch_size):
        batch_fnames = items[batch_start:batch_start + batch_size]
        batch_images = [images_by_fname[fname] for fname in batch_fnames]
        batch_prompts = [embedded[fname]["dino_prompt"] for fname in batch_fnames]

        # First pass: single forward pass for whole batch.
        if oom_seen:
            first_pass = [None] * len(batch_fnames)
            per_item_lat = 0.0
        else:
            t0 = time.perf_counter()
            try:
                first_pass = detector.detect_batch(batch_images, batch_prompts)
            except torch.OutOfMemoryError:
                oom_seen = True
                if torch.cuda.is_available():
                    torch.cuda.empty_cache()
                first_pass = [None] * len(batch_fnames)
                print("  [warn] detection OOM on batch; marking remaining as unavailable")
            except Exception as exc:
                first_pass = [None] * len(batch_fnames)
                print(f"  [warn] detection batch failed; falling back to per-image ({exc})")
            per_item_lat = (time.perf_counter() - t0) / max(1, len(batch_fnames))

        for i, fname in enumerate(batch_fnames):
            det = first_pass[i]
            used_prompt = None
            lat_detect = per_item_lat

            # Serial fallback (second-pass templates + Ollama) only for missed images.
            if det is None and not oom_seen:
                t_fallback = time.perf_counter()
                try:
                    det, used_prompt = _detect_with_fallback(
                        detector, batch_images[i], batch_prompts[i], settings
                    )
                except torch.OutOfMemoryError:
                    oom_seen = True
                    if torch.cuda.is_available():
                        torch.cuda.empty_cache()
                    print("  [warn] detection OOM during fallback; marking remaining as unavailable")
                lat_detect = per_item_lat + (time.perf_counter() - t_fallback)
...
def _eval_yoloe_detection(
    ...
    results: Dict[str, dict] = {}
    items = list(test_set)
    images = [load_image(embedded[fname]["image_path"]) for fname in items]

    batch_size = _YOLOE_DETECT_BATCH_SIZE
    for batch_start in range(0, len(items), batch_size):
        batch_fnames = items[batch_start:batch_start + batch_size]
        batch_images = images[batch_start:batch_start + batch_size]
        t0 = time.perf_counter()
        try:
            batch_results = detector.detect_all_batch(batch_images)
        except Exception as exc:
            print(f"  [warn] YOLOE batch detection failed: {exc}; falling back to per-image")
            batch_results = []
            for img in batch_images:
                try:
                    batch_results.append(detector.detect_all(img))
                except Exception as exc2:
                    print(f"  [warn] YOLOE per-image fallback failed: {exc2}")
                    batch_results.append((None, None))
        batch_lat = time.perf_counter() - t0
        per_item_lat = batch_lat / max(1, len(batch_images))

        for fname, (boxes, scores) in zip(batch_fnames, batch_results):
...
_INFERENCE_TRACE_FIELDS = [
    "image", "label", "top_k_rank", "top_3_hit", "top_5_hit", "top_10_hit",
    "personalized_matched", "personalized_match_label", "personalized_correct",
    "img_sim_pe", "txt_sim_pe", "personalized_similarity",
    "failure_class", "lat_retrieve_pe",
]


def _write_inference_traces(retrieval: List[dict]) -> None:
    traces_path = _BENCHMARKS_DIR / "inference_traces.csv"
    rows = []
    for r in retrieval:
        rows.append({
            "image": r["image"],
            "label": r["label"],
            "top_k_rank": r.get("top_k_rank", 0),
            "top_3_hit": r.get("top_3_hit", 0),
            "top_5_hit": r.get("top_5_hit", 0),
            "top_10_hit": r.get("top_10_hit", 0),
            "personalized_matched": r.get("personalized_matched", 0),
            "personalized_match_label": r.get("personalized_match_label", ""),
            "personalized_correct": r["personalized_correct"],
            "img_sim_pe": round(r.get("img_sim_pe", 0.0), 6),
            "txt_sim_pe": round(r.get("txt_sim_pe", 0.0), 6),
            "personalized_similarity": round(r["personalized_similarity"], 6),
            "failure_class": _classify_failure(r),
            "lat_retrieve_pe": round(r["lat_retrieve_pe"], 6),
        })
    with open(traces_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=_INFERENCE_TRACE_FIELDS)
        writer.writeheader()
        writer.writerows(rows)
    print(f"Inference traces written to: {traces_path}")


def _write_failure_taxonomy(retrieval: List[dict]) -> None:
    counts: dict = {}
    for r in retrieval:
    ...
    n = max(len(retrieval), 1)
    taxonomy = {
        cls: {"count": cnt, "rate": round(cnt / n, 4)}
        for cls, cnt in sorted(counts.items())
    }
    tax_path = _BENCHMARKS_DIR / "failure_taxonomy.json"
    with open(tax_path, "w") as f:
        json.dump({"n": n, "taxonomy": taxonomy}, f, indent=2)
    print(f"Failure taxonomy written to: {tax_path}")


def _generate_operating_points(similarity_stats: List[dict]) -> List[dict]:
    """Sweep personalized threshold and return operating point metrics per step.

    Each row: threshold, recall, precision, abstention_rate, fp_rate.
    Used by Branch A to select operating point without running a full re-benchmark.

    recall    = fraction of samples where true label is rank-1 accepted match
    precision = recall / accepted_match_rate (fraction of accepted matches that are correct)
    fp_rate   = fraction of samples where a wrong label is accepted
    """
    if not similarity_stats:
        return []
    n = len(similarity_stats)
    points: List[dict] = []
    for t_raw in np.arange(0.10, 0.96, 0.01):
        t = float(round(t_raw, 3))
        true_positive = 0
        false_positive = 0
        for s in similarity_stats:
            best_true = float(s.get("best_true_pe", -1.0))
            best_wrong = float(s.get("best_wrong_pe", 0.0))
            if best_true >= t and best_true > best_wrong:
                true_positive += 1
            elif best_wrong >= t and best_wrong >= best_true:
                false_positive += 1
        accepted = true_positive + false_positive
        points.append({
            "threshold": t,
            "recall": round(true_positive / n, 4),
            "precision": round(true_positive / max(accepted, 1), 4),
            "accepted_match_rate": round(accepted / n, 4),
            "abstention_rate": round(1.0 - accepted / n, 4),
            "fp_rate": round(false_positive / n, 4),
        })
    return points


def _compute_fixed_fp_budget(
    similarity_stats: List[dict],
    targets: Optional[List[float]] = None,
) -> dict:
    """Find the highest recall threshold that satisfies each FP rate budget target.

    Returns a dict mapping each target to the best operating point at that FP budget.
    Mirrors the NIST-style evaluation described in ACCURACY_HARDENING_PLAN.log.
    """
    if targets is None:
        targets = [0.05, 0.10]
    if not similarity_stats:
        return {}
    operating_points = _generate_operating_points(similarity_stats)
    result: dict = {}
    for target_fp in targets:
        feasible = [p for p in operating_points if p["fp_rate"] <= target_fp]
        if not feasible:
            result[f"fp_budget_{int(target_fp * 100):02d}pct"] = {
                "target_fp_rate": target_fp,
                "satisfied": False,
                "note": "no threshold satisfies this FP budget",
            }
        else:
            best = max(feasible, key=lambda p: (p["recall"], -p["threshold"]))
            result[f"fp_budget_{int(target_fp * 100):02d}pct"] = {
                "target_fp_rate": target_fp,
                "satisfied": True,
                ...
                "accepted_match_rate": best["accepted_match_rate"],
                "actual_fp_rate": best["fp_rate"],
            }
    return result


def _write_debug_dump(retrieval: List[dict]) -> None:
    """Write top-20 FPs and top-20 FNs to benchmarks/debug_dump.txt.

    FPs: wrong_label_match rows sorted by similarity descending (hardest FPs).
    FNs: rows where true label was in top-10 but no match was returned, sorted
         by top_k_rank ascending (closest near-misses).
    """
    dump_path = _BENCHMARKS_DIR / "debug_dump.txt"
    fps = sorted(
        [r for r in retrieval if _classify_failure(r) == "wrong_label_match"],
        key=lambda r: r.get("personalized_similarity", 0.0),
        reverse=True,
    )[:20]
    fns = sorted(
        [
            r for r in retrieval
            if _classify_failure(r) in {"threshold_miss", "near_miss"}
        ],
        key=lambda r: int(r.get("top_k_rank", 99)),
    )[:20]
    lines: List[str] = []
    lines.append("=== TOP-20 FALSE POSITIVES (wrong_label_match, highest similarity first) ===")
    if fps:
        for i, r in enumerate(fps, 1):
            lines.append(
                f"  {i:2d}. image={r['image']}  true={r['label']}"
                f"  matched={r.get('personalized_match_label','')}  sim={r.get('personalized_similarity',0):.4f}"
            )
    else:
        lines.append("  (none)")
    lines.append("")
    lines.append("=== TOP-20 FALSE NEGATIVES (threshold_miss / near_miss, closest rank first) ===")
    if fns:
        for i, r in enumerate(fns, 1):
            lines.append(
                f"  {i:2d}. image={r['image']}  true={r['label']}"
                f"  rank={r.get('top_k_rank',0)}  class={_classify_failure(r)}"
                f"  sim={r.get('personalized_similarity',0):.4f}"
            )
    else:
        lines.append("  (none)")
    with open(dump_path, "w") as f:
        f.write("\n".join(lines) + "\n")
    print(f"Debug dump written to: {dump_path}")
...
def _write_output(
    ...
    hardening_metrics: dict,
    retrieval_rows: List[dict],
    operating_points: Optional[List[dict]] = None,
    fixed_fp_budget: Optional[dict] = None,
    alias_candidates: Optional[List[tuple]] = None,
) -> None:
    ...
    metadata = {
        ...
        "fixed_fp_budget": fixed_fp_budget or {},
        "alias_candidates": [
            {"label_a": la, "label_b": lb, "max_sim": round(sim, 4)}
            for la, lb, sim in (alias_candidates or [])
        ],
        "timestamp": datetime.now(timezone.utc).isoformat(),
    }
    _write_inference_traces(retrieval_rows)
    _write_failure_taxonomy(retrieval_rows)
    _write_debug_dump(retrieval_rows)
    if operating_points:
        op_path = _BENCHMARKS_DIR / "operating_points.csv"
        with open(op_path, "w", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=list(operating_points[0].keys()))
            writer.writeheader()
            writer.writerows(operating_points)
        print(f"Operating points written to: {op_path}")
    if fixed_fp_budget:
        fp_budget_path = _BENCHMARKS_DIR / "fixed_fp_budget.json"
        with open(fp_budget_path, "w") as f:
            json.dump(fixed_fp_budget, f, indent=2)
        print(f"Fixed FP budget written to: {fp_budget_path}")
...
def _print_summary(
    ...
    if hardening_metrics:
        print()
        print("--- Top-k Recall (personalized, unthresholded) ---")
        print(f"  Top-3 recall:      {hardening_metrics['top_3_recall']:.1%}")
        print(f"  Top-5 recall:      {hardening_metrics['top_5_recall']:.1%}")
        print(f"  Top-10 recall:     {hardening_metrics['top_10_recall']:.1%}")
        print(f"  Accepted rate:     {hardening_metrics['accepted_match_rate']:.1%}")
        print(f"  Accepted prec.:    {hardening_metrics['accepted_precision']:.1%}")
        print(f"  Abstention rate:   {hardening_metrics['abstention_rate']:.1%}")
        print(f"  Retrieve lat p50:  {hardening_metrics['latency_p50_retrieve_pe']*1000:.1f}ms")
        print(f"  Retrieve lat p95:  {hardening_metrics['latency_p95_retrieve_pe']*1000:.1f}ms")
...
def main() -> None:
    ...
    if args.ocr_threshold is not None:
        settings.ocr_text_likelihood_threshold = args.ocr_threshold
    ...
    alias_candidates = detect_alias_candidates(database)
    if alias_candidates:
        print(f"  alias candidates: {len(alias_candidates)} label pairs above similarity 0.92")
        for la, lb, sim in alias_candidates:
            print(f"    {la} <-> {lb}: {sim:.3f}")
    ...
    operating_points = _generate_operating_points(similarity_stats)
    fixed_fp_budget = _compute_fixed_fp_budget(similarity_stats)
    ...
    hardening_metrics = _compute_hardening_metrics(retrieval_results)

    # Phase 6: Detection
    _enforce_memory_guard(monitor, "phase 6")
    print("\n[6/7] Evaluating YOLOE scan detector and GroundingDINO detection...")
    if args.no_detection:
        print("  [info] detector evaluation skipped by --no-detection")
        yoloe_results = {}
        dino_results = {}
    else:
    ...
    _write_output(
        ...
        threshold_tuning=threshold_tuning,
        hardening_metrics=hardening_metrics,
        retrieval_rows=retrieval_results,
        operating_points=operating_points,
        fixed_fp_budget=fixed_fp_budget,

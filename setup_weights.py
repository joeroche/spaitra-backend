...
def _download_hf_model(repo_id: str, size_hint: str, gated: bool = False):
    ...
    try:
        ...
        if repo_id == DINOV3_MODEL:
            _ensure_dinov3_processor_alias(Path(path))
        print(f"  -> {path}")
...
def _ensure_dinov3_processor_alias(snapshot_path: Path) -> None:
    """Transformers may look for processor_config.json instead of preprocessor_config.json."""
    pre = snapshot_path / "preprocessor_config.json"
    dst = snapshot_path / "processor_config.json"
    if not pre.exists() or dst.exists():
        return
    try:
        dst.symlink_to(pre.name)
    except OSError:
        dst.write_text(pre.read_text())
